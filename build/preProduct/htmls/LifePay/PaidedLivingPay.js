 ...
 *生活缴费类型具体缴费功能
 */
ibsapp.register.controller("PaidedLivingPayCtrl", ["$state", "$stateParams", "$location", "$timeout", "$filter", "$rootScope", "$targets", '$scope', '$remote',
function($state, $stateParams, $location,  $timeout, $filter, $rootScope, $targets, $scope, $remote) {
	//创建生活缴费类型图标滑动块对象
	$($timeout(function() {
	...
	$scope.init = function() {
		...
		promise.then(function(promiseData) {
			...
			if (promiseData == "T") {
				//审核中
				$rootScope.ForceAuthText = "实名认证正在审核中，暂时无法进行缴费功能查询！";
				$rootScope.ForceYesBtn = "立即查看";
				$("div#forceNeedAuto_id").show();
			}
			if (promiseData != "T" && promiseData != "N") {
				//需要认证
				$rootScope.ForceAuthText = "您未进行实名认证，暂时无法进行缴费功能查询!";
	...
	$scope.qryChargeBusiType = function() {
		...
		$remote.post("ChargeBusiTypeQry.do", formdata, function(data) {
			...
			//改变城市后，需重新查询当前城市的可缴费的业务种类，默认选择缴费类型第一项，再查询其下的缴费项目
			if ($scope.BusiType.BUSI_TYPE == undefined || $scope.BusiType.BUSI_TYPE == '') {
				$scope.BusiType.BUSI_NAME = $scope.BusiTypeList[0].BUSI_NAME;
				$scope.BusiType.BUSI_TYPE = $scope.BusiTypeList[0].BUSI_TYPE;
				//NativeCall.setTitle($scope.BusiType.BUSI_NAME);
			}
			//查询某缴费类型下具体的可缴费项目
	...
	//某缴费类型下有多项缴费项目时，可选择缴费项目
	$scope.choosePayProj = function(data) {
		$scope.PayProjObj = data;
		//当前缴费项目
		$("#payproj").text(data.ITERM_NAME);
		...
		//清除上一缴费项目的下拉框
		$(".outList:not(#payprojList,#paymentList)").remove();
	};
	//重新选择缴费选项
	$scope.reQryPayProj = function(data) {
		$scope.BusiType.BUSI_NAME = data.BUSI_NAME;
		$scope.BusiType.BUSI_TYPE = data.BUSI_TYPE;
		//$rootScope.prdId = undefined;
		//重新选择时清除客户端发送的信息
		//查询该缴费类型下的缴费项目
		//NativeCall.setTitle(data.BUSI_NAME);
		$scope.qryPayProjList();
	};
	//查询缴费项目参数
	...
	$scope.doIt2 = function() {
		...
		$remote.post("FaceAuthResultQry.do", formData, function(data) {
			...
			if ($scope.Status == 222) {
				...
				//认证成功&日累计超限
				popup_text = "日累计缴费限额为" + data.PerDayLimitY + "元，您已超过限额！</p>";
				$scope.AlertErr(popup_text);
				return;
			} else {
				//已通过 或 未通过但限额充足
